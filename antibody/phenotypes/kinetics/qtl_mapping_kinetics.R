...
dat.7_10=read.csv(paste("data_bin/kinetics/dat.7_10.",data.type,".csv",sep=""))
dat.10_15=read.csv(paste("data_bin/kinetics/dat.10_15.",data.type,".csv",sep=""))
dat.15_45=read.csv(paste("data_bin/kinetics/dat.15_45.",data.type,".csv",sep=""))
...
col.list=colnames(dat.7_10)


#### lists ####
day.list=list("7-10","10-15","15-45")
dat.list=list(dat.7_10,dat.10_15,dat.15_45)

#### phenotypic distribution plots ####

for(k in 1:length(day.list))
{
  day=day.list[[k]]
  dat=dat.list[[k]]
  ...
  for(i in 1:6)
  {
    ...
    g+geom_point()+theme_classic()+
      labs(title=paste("Day",day.list[[k]],abs[i],sep=" "),y="RIX",x=paste0("log ",data.type))
    
    ggsave(file.path(paste0("phenotypes/kinetics/pheno_dist"),paste0("pheno_dist_d",day.list[[k]],"_",abs[i],"_",data.type,".jpg")), width=6, height=9)
  }
}
...
#             file.path(paste("phenotypes/d",day,sep=""),
#                       paste("heritability_estimates_",data.type,"_d",day.list[[k]],".csv",sep="")))
#   heritability.bounds=NULL
# }
...
for(k in 1:length(day.list))
{
  ...
  jpeg(file.path(paste0("phenotypes/kinetics/correlation_matrix_",data.type,"_d",day.list[[k]],".jpg")))
...
for(k in 1:length(day.list))
{
  ...
  png(file.path(paste0("phenotypes/kinetics/qtl_scans/qtl_scan_d",day,"_",data.type,".png"))
  ...
  for(i in 1:6)
  {
    ...
    save(qtl,file=file.path(paste0("phenotypes/kinetics/qtl_scans/d",day,"_",abs[i],"_",
...
for(k in 1:length(day.list))
{
  ...
  png(file.path(paste0("phenotypes/kinetics/qtl_scans/qtl_scan_d",day,"_",data.type,
                       "_perms_",nperm,".png")),width=1000,height=500)
  par(mfrow = c(2,3))
  for(i in 1:6)
  {
    pheno=abs[i]
    load(file.path(paste0("phenotypes/kinetics/qtl_scans/d",day,"_",abs[i],"_",
                          data.type,"_qtl_obj",".RData")))
    perms = scanone.perm(pheno = Map.dat, pheno.col = abs[i], probs = model.probs, addcovar = covar, snps= MM_snps, nperm = nperm)
    save(perms,file=file.path(paste0("phenotypes/kinetics/qtl_scans/d",day,"_",abs[i],"_",
...
day="15-45"
pheno="IgG2b"
load(file=file.path(paste0("phenotypes/kinetics/qtl_scans/d",day,"_",pheno,"_",
                    data.type,"_qtl_obj",".RData",sep="")))

chr=5

# plot(qtl, main=paste("Day",day,pheno,sep=" "))
coefplot(qtl, chr=chr,main=paste("Day",day,pheno,"Chromosome",chr,sep=" "),legend=F)
# coefplot(qtl, chr = "X", legend=F, sex="F")
...
load(file=file.path(paste0("phenotypes/kinetics/qtl_scans/d",day,"_",
  pheno,"_",data.type,"_perms_",nperm,".Rdata",sep="")))
